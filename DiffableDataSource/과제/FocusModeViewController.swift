...
struct FocusSettingItem: Hashable, Identifiable {
...
class FocusModeViewController: UIViewController {
    ...
    // 3.3. 데이터 소스 변수 선언
    var dataSource: UICollectionViewDiffableDataSource<Section, FocusSettingItem>!
    
    // 3.4. 초기 데이터 리스트
    let list = [
        FocusSettingItem(name: "방해 금지 모드", detail: nil, icon: "moon.fill"),
        FocusSettingItem(name: "수면", detail: nil, icon: "bed.double.fill"),
        FocusSettingItem(name: "업무", detail: "09:00 - 06:00", icon: "briefcase.fill"),
        FocusSettingItem(name: "개인 시간", detail: nil, icon: "person.fill"),
    ]
    ...
    private func configureDataSource() {
        var registration: UICollectionView.CellRegistration<UICollectionViewListCell, FocusSettingItem>!
    ...
    private func updateSnapshot() {
        var snapshot = NSDiffableDataSourceSnapshot<Section, FocusSettingItem>()
        snapshot.appendSections(Section.allCases)
        snapshot.appendItems(list, toSection: .main)
        
        snapshot.appendItems([FocusSettingItem(name: "모든 기기에서 공유", detail: "하이", icon: "star.fill")], toSection: .sub)
